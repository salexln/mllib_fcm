...
class FuzzyCMeans private (
  ...
  extends KMeans() {
  // } c, maxIterations, runs, initializationMode, initializationSteps, epsilon ) {


  def this() = this(2, 20, 1, KMeans.K_MEANS_PARALLEL, 5, 1e-4, Utils.random.nextLong(), 1e-4)

  def getFuzzynessCoefficient: Double = fuzzynessCoefficient

  def setFuzzynessCoefficient(fuzzyness: Double) : this.type = {
    this.fuzzynessCoefficient = fuzzyness
    this
  }
